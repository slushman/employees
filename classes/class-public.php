...
class Employees_Public {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( dirname( __FILE__ ) ) . 'assets/css/employees-public.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( dirname( __FILE__ ) ) . 'assets/js/employees-public.js', array( 'jquery' ), $this->version, false );
	...
	public function loop_template_from_shortcode( $args ) {
		...
		foreach ( $args['show'] as $item ) {
			...
			add_action( 'employees-loop-content', array( $plugin_templates, 'loop_content_' . $item ), $priority, 2 );
		...
		if ( ( in_array( 'container_begin', $args['show'] ) || in_array( 'box_begin', $args['show'] ) ) && ( ! in_array( 'container_end', $args['show'] ) || ! in_array( 'box_end', $args['show'] ) ) ) {
			...
			add_action( 'employees-loop-content', array( $plugin_templates, 'loop_content_container_end' ), $priority + 5, 2 );

		}

		if ( in_array( 'link_begin', $args['show'] ) && ! in_array( 'link_end', $args['show'] ) ) {

			add_action( 'employees-loop-content', array( $plugin_templates, 'loop_content_link_end' ), $priority + 5, 2 );
	...
	public function remove_links_in_loop_template( $args ) {
		...
		remove_action( 'employees-before-loop-content', array( $plugin_templates, 'loop_content_link_begin' ), 15, 2 );
		remove_action( 'employees-after-loop-content', array( $plugin_templates, 'loop_content_link_end' ), 10, 2 );
	 ...
	 */
	public function shortcode_employeelist( $atts = array() ) {
	...
	} // shortcode_employeelist()

	/**
	 * Processes shortcode employeesnav
	 *
	 * @param 	array 	$atts 		Shortcode attributes
	 * @return	mixed	$output		Output of the buffer
	 */
	public function shortcode_employeesnav( $atts = array() ) {

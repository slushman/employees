...
if ( ! function_exists( 'employees_templates' ) ) {
	...
	function employees_templates() {
		...
		return Employees\Frontend\Templates::this();
...
class Templates {
	...
	public function hooks () {
		...
		// Loop
		add_action( 'employees_before_loop', 			array( $this, 'loop_nav' ), 10, 1 );
		add_action( 'employees_before_loop', 			array( $this, 'loop_wrap_start' ), 10, 1 );
		add_action( 'employees_before_loop_content', 	array( $this, 'loop_content_wrap_begin' ), 10, 2 );
		add_action( 'employees_before_loop_content', 	array( $this, 'loop_content_link_begin' ), 15, 2 );
		add_action( 'employees_after_loop_content', 	array( $this, 'loop_content_link_end' ), 10, 2 );
		add_action( 'employees_after_loop_content', 	array( $this, 'loop_content_wrap_end' ), 90, 2 );
		add_action( 'employees_after_loop', 			array( $this, 'loop_wrap_end' ), 10, 1 );


		// Single
		add_action( 'employees_single_content', array( $this, 'single_employee_thumbnail' ), 10, 2 );
		add_action( 'employees_single_content', array( $this, 'single_employee_name' ), 15, 2 );
		add_action( 'employees_single_content', array( $this, 'single_employee_job_title' ), 20, 2 );
		add_action( 'employees_single_content', array( $this, 'single_employee_description' ), 25, 2 );
		add_action( 'employees_single_content', array( $this, 'single_employee_contact_info' ), 30, 2 );
		add_action( 'employees_single_content', array( $this, 'single_employee_comms' ), 35, 2 );
	...
	public function loop_content_social_links( $item, $meta = array() ) {
		...
		$links = array( 'facebook', 'flickr', 'github', 'google', 'instagram', 'linkedin', 'medium', 'pinterest', 'tumblr', 'twitter', 'website', 'wordpress' );
		$links = apply_filters( 'employees-links-list', $links );
	...
	private function set_options() {
		...
		$this->options = get_option( EMPLOYEES_SETTINGS );
